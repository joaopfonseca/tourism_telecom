    ...
    .map(key => {
      return (
        <div className="list-item" key={`list-item-${key}`}>
          <div className="museum-name">{dataDict[key].fullname}</div>
      ...
      return (
        <div>
          ...
          <p className="description">
            Total visits is the number of people whose phones were connected to NOS network in the location.
          </p>
          <div className="total-visits">
            <div className="museum-name"><center>Total Visits:</center></div>
            <div className="museum-number"><center>{parseInt(totalFcVisits).toLocaleString()}</center></div>
          </div>
        </div>
      );
    }
